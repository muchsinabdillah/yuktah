...
class TrsLearningUjiKompetensiController extends Controller
{
    use ResponseAPI;
    private $repository; 
    private $repositoryPractice; 
    private $repositoryUser; 
    private $repositoryLearningStream; 
    public function __construct(  
        TrsLearningPracticeRepositoryInterface $repository,
        LearningPracticeRepositoryInterface $repositoryPractice,
        ...
        )
    {
        ...
        $this->repositoryPractice = $repositoryPractice; 
    ...
    public function store(Request $request)
    {
        ...
        try { 
            ...
            $kuisionerdata = $this->repositoryPractice->allPerLearningUuid($request->learningdetailuuid);  
            if($kuisionerdata->count() < 1){  
                return $this->error('Data Uji Kompetensi di tahun ini tidak ditemukan.', [],400);
            } 
            ...
            return $this->success('Uji Kompetensi Berhasil di buat.', $metadata, 201);
    ...
    public function edit(Request $request)
    {
        ...
        try { 
            ...
            if($user->count() < 1){  
                return $this->error('Data Transaksi Uji Kompetensi tidak ditemukan.', [],400);
            } 

            $valnull = $this->repository->findNullDatabyUuidDetail($request->uuid);  
         
            if($valnull->count() > 0){  
                return $this->error('Terdapat Data Uji Kompetensi Masih Kosong, Silahkan isi Semua Data.', [],400);
            } 
    ...
    public function update(Request $request )
    {
        ...
        try { 
            ...
            if($user->count() < 1){  
                return $this->error('Data Uji Kompetensi tidak ditemukan.', [],400);
            } 
             $question = $this->repositoryPractice->findbyid($request->uuidquestion); 
              
            if($question->count() < 1){  
                return $this->error('Data Soal Uji Kompetensi tidak ditemukan.', [],400);
            } 
           ...
           $this->repository->updateItemDetailAnswer($dataArray); 
            ...
            return $this->success('Data Uji Kompetensi berhasil diinsput.',[], 201);
