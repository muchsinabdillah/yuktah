...
class RegencieController extends Controller
{
    use ResponseAPI;
    private $repository;
    public function __construct(RegencieRepositoryInterface $repository)
    {
    ...
    public function store(Request $request)
    {
        ...
        try { 
            ...
            if($execute){
                return $this->success('Regencies retrieved successfully', $data, 201);
            }else{
                return $this->error('Regencies retrieved failure', 400);
            }
        } catch (\Exception $e) {
            DB::rollBack();
            return $this->error($e->getMessage(), $e->getCode());
    ...
    public function update(Request $request)
    {
        ...
        $execute = $this->repository->findbyid($request->uuid);  
            if($execute->count() < 1){  
                return $this->error('Regencie Not Found.', [],400);
            } 
        ...
        try {
            ...
            DB::commit(); 
            if($executes){
                return $this->success('Regencies updated successfully', []);
            } 
        } catch (\Exception $e) {
            DB::rollBack();
            return $this->error($e->getMessage(), $e->getCode());
