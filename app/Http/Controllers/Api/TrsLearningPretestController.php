...
//a
class TrsLearningPretestController extends Controller
{
    use ResponseAPI;
    private $repository; 
    ...
    public function store(Request $request)
    {
        ...
        try { 
            ...
            if($kuisionerdata->count() < 1){  
                return $this->error('Data Pre Test di tahun ini tidak ditemukan.', [],400);
            } 
            ...
            return $this->success('Pretest Berhasil di buat.', $metadata, 201);
    ...
    public function edit(Request $request)
    {
        ...
        try { 
            ...
            if($user->count() < 1){  
                return $this->error('Data Transaksi Pretest tidak ditemukan.', [],400);
            } 

            $valnull = $this->repository->findNullDatabyUuidDetail($request->uuid);  
         
            if($valnull->count() > 0){  
                return $this->error('Terdapat Data Pretest Masih Kosong, Silahkan isi Semua Data.', [],400);
            } 
    ...
    public function update(Request $request )
    {
        ...
        try { 
            ...
            if($user->count() < 1){  
                return $this->error('Data Pretest tidak ditemukan.', [],400);
            } 
             $question = $this->repositoryQuestion->findbyid($request->uuidquestion); 
              
            if($question->count() < 1){  
                return $this->error('Data Soal Pretest tidak ditemukan.', [],400);
            } 
           ...
           $this->repository->updateItemDetailAnswer($dataArray); 
            ...
            return $this->success('Data Pretest berhasil diinsput.',[], 201);
