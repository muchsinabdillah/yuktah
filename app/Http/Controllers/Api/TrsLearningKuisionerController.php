...
class TrsLearningKuisionerController extends Controller
{
    use ResponseAPI; 
    private $repository; 
    private $repositoryKuisoner; 
    private $repositoryUser; 
    private $repositoryLearningStream; 
    public function __construct(  
        TrsLearningKuisionerRepositoryInterface $repository,
        LearningQuestionNairRepositoryInterface $repositoryKuisoner,
        ...
        )
    {
        ...
        $this->repositoryKuisoner = $repositoryKuisoner; 
    ...
    public function store(Request $request)
    {
        ...
        try { 
            ...
            $kuisionerdata = $this->repositoryKuisoner->allPerLearningUuid($request->learningdetailuuid);  
            if($kuisionerdata->count() < 1){  
                return $this->error('Data Kuisioner di tahun ini tidak ditemukan.', [],400);
            } 
            ...
            return $this->success('Kuisioner berhasil dibuat.', $metadata, 201);
    ...
    public function edit(Request $request)
    {
        ...
        try { 
            ...
            if($user->count() < 1){  
                return $this->error('Data Transaksi Kuisioner tidak ditemukan.', [],400);
            } 

            $valnull = $this->repository->findNullDatabyUuidDetail($request->uuid);  
         
            if($valnull->count() > 0){  
                return $this->error('Terdapat Data Kuisinoer Masih Kosong, Silahkan isi Semua Data.', [],400);
            } 
            $kuisionerdata = $this->repositoryKuisoner->allPerLearningUuid($request->learninguuid);  
            if($kuisionerdata->count() < 1){  
                return $this->error('Data Kuisioner di tahun ini tidak ditemukan.', [],400);
            }  
            $sumkuisionerScore = $this->repository->sumkuisionerfinal($request->uuid)->first();
            $grantotalnilai = round($sumkuisionerScore->total/$kuisionerdata->count())*10;
            
            $this->repository->updateFinalScoreStreamDetail($request->streamuuiddetail,$grantotalnilai); 
            $this->repository->updateFinalScoreKuisioner($request->uuid,$grantotalnilai); 

            DB::commit();
           
            
            return $this->success('Data Kuisioner Selesai.', [], 201);
    ...
    public function update(Request $request)
    {
        ...
        try { 
            ...
            if($user->count() < 1){  
                return $this->error('Data Kuisioner tidak ditemukan.', [],400);
            } 
           ...
           $this->repository->updateItemDetailAnswer($dataArray); 
            ...
            return $this->success('Data Kuisioner berhasil diinput.', [], 201);
