...
class CarrierSpecialistController extends Controller
{
    use ResponseAPI;
    private $repository;
    public function __construct(CarrierSpecialistRepositoryInterface $repository)
    {
    ...
    public function update(Request $request)
    {
        ...
        $execute = $this->repository->findbyid($request->uuid);  
            if($execute->count() < 1){  
                return $this->error('Carrier Specialist Not Found.', [],400);
            } 
        ...
        try {
            ...
            DB::commit(); 
            if($executes){
                return $this->success('Carrier Specialists updated successfully', []);
            } 
        } catch (\Exception $e) {
            DB::rollBack();
            return $this->error($e->getMessage(), $e->getCode());
